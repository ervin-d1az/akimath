...
redesign. The band value set's default is `{under_13, 13_17, 18_plus}`; the input is a neutral date
...
| **D13** | **Google and Apple sign-in come off `1.1`, along with the `O` divider.** | The reasoning is Play's Families policy restricting SDKs in child-directed apps, plus the audience constraint in `CLAUDE.md`:7 — **not** a straight DEP-1 citation, because a redirect-based OAuth flow through Better Auth is not necessarily an SDK that phones home, and a mis-cited NEVER is its own problem in a repo where reviewers cite rule IDs. |
